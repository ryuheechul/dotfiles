...
##   - that being said, in normal situation you shouldn't need to run this that often
##     - since it survives rebooting and WiFi changes (`nmcli dev wifi connect ...`), etc.
...
echo $networks | jq -c '.[]' | while read network; do
  ...
  if test -z "$domain" || test -z "$dns_list" || test -z "$interface"; then
    ...
    set -x
    sudo resolvectl revert $interface
    sudo resolvectl dns $interface $(echo $dns_list)
