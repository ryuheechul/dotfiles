...
# declarative alternative to `deno install --global --allow-all npm:backlog.md`:
...
# - `deno cache --reload backlog.md` may be the way to update? (let me test that later)
viaDeno {
  pkgs = pkgs;
  pkg-name = "backlog";
  deno-pkg-name = "npm:backlog.md";
}
