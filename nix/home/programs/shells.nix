...
in
{
  ...
  programs.zsh = {
    ...
    # Stay with legacy behavior ($HOME instead of $XDG_CONFIG_HOME/zsh) until I want to change later.
    dotDir = config.home.homeDirectory;
    # # switch to below when I want to keep zsh files under $XDG_CONFIG_HOME/zsh
    ...
    initContent = pkgs.lib.mkMerge [
      # replaces deprecated initExtraFirst (pkgs.lib.mkBefore = pkgs.lib.mkOrder 500)
      (pkgs.lib.mkBefore ''
      ...
      '')
      # replaces deprecated initExtraBeforeCompInit (pkgs.lib.mkOrder 550, just after compinit guard)
  ...
  programs.nushell = {
    ...
    extraConfig = ''
      const native_config_file = "${my_dot_d}/nushell/config.nu"
